...
while(true){
    let findMember = prompt( `${member} \n 수정할 멤버 이름을 적어주세요`);
    let index = member.indexOf(findMember);

    if(member.includes(findMember)){ 
        let seMember = prompt('새로 추가 할 멤버');
        member.splice(`${index}`,1);
        member.push(seMember);
    }else{           
        alert(`${findMember}는 잘못된 이름입니다.`);
    }
